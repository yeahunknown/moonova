...
export const LaunchGuideModal = ({ isOpen, onClose }: LaunchGuideModalProps) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="fixed inset-0 z-50 w-screen h-screen sm:left-[50%] sm:top-[50%] sm:w-[95vw] sm:max-w-4xl sm:h-[95vh] sm:max-h-[800px] sm:translate-x-[-50%] sm:translate-y-[-50%] border-0 sm:border border-border bg-background shadow-2xl animate-scale-in sm:rounded-lg overflow-hidden flex flex-col p-0">
        {/* Header */}
        <div className="flex-shrink-0 p-4 border-b border-border bg-background flex items-center justify-between">
          <h2 className="text-lg sm:text-xl font-semibold text-foreground">Launch Guide</h2>
          <button 
            onClick={onClose}
